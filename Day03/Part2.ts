...
const answer = identifiers
    .map(item => [
        grid.find(gridItem => gridItem.x === item.x && gridItem.y === item.y - 1),
        grid.find(gridItem => gridItem.x === item.x && gridItem.y === item.y + 1),
        grid.find(gridItem => gridItem.x === item.x - 1 && gridItem.y === item.y),
        grid.find(gridItem => gridItem.x === item.x + 1 && gridItem.y === item.y),
        grid.find(gridItem => gridItem.x === item.x - 1 && gridItem.y === item.y - 1),
        grid.find(gridItem => gridItem.x === item.x + 1 && gridItem.y === item.y - 1),
        grid.find(gridItem => gridItem.x === item.x - 1 && gridItem.y === item.y + 1),
        grid.find(gridItem => gridItem.x === item.x + 1 && gridItem.y === item.y + 1),
    ])
    .map(item => item.filter(item => item !== undefined && item.value.ints().length > 0))
    .map(item => {
        const subItems = item
            .reduce((acc ,state) => {
